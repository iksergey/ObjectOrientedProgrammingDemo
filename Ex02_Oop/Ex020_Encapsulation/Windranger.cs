using Ex020_Encapsulation.Infrastructure;
using System;
...
namespace Ex020_Encapsulation
{
    class Windranger
    {
        ...
        public ushort MovementSpeed => (ushort)(movementSpeed + agility * 0.02);

        public Windranger() :
            this(name: $"Hero #{Guid.NewGuid().ToString().Substring(0, 5)}",
                 strength: Randomize.Value(10, 15),
                 agility: Randomize.Value(20, 25),
                 intelligence: Randomize.Value(10, 15),
                 level: 1,
                 movementSpeed: Randomize.Value(),
                 health: Randomize.Value(koff: 2),
                 mana: Randomize.Value()
                )
        {
            Items = new Backpack();
            Attributes = HeroType.Agility;
        }

        public Windranger(string name,
                          ushort strength,
                          ushort agility,
                          ushort intelligence,
                          byte level,
        ...
        {
            if (this.Items["PortalItem"] != "NullItem")
            {
